...
public class NetworkSimulator {
	...
	void init() {
		...
		/***************************
		 * Initialize connection costs between routers Used for performing some
		 * sanity checks in the tolayer2() method
		 ***************************/
		...
		while (true) {
			...
			if (TRACE >= 2) {
				...
				if (nextEvent.getType() == FROM_LAYER2) {
					...
					System.out.printf(" contents: %3d %3d %3d %3d\n", nextEvent.getPacket().mincost[0],
							nextEvent.getPacket().mincost[1], nextEvent.getPacket().mincost[2],
							nextEvent.getPacket().mincost[3]);
				}
	...
	public static void tolayer2(Packet packet) {
		 ...
		 * decide to do something with the packet after we return back to
		 * him/her
		 *************************************************/
		Packet mypktcopy = new Packet(packet);
		if (TRACE > 2) {
			System.out.printf("    TOLAYER2: source: %d, dest: %d\n              costs:", mypktcopy.sourceid,
					mypktcopy.destid);
			for (int i = 0; i < 4; i++)
