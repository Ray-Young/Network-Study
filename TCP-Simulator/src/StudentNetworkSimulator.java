...
public class StudentNetworkSimulator extends NetworkSimulator
{
    ...
    private int lastRCV;
    private int count;
    private int space;		//enough space in the window
		...
		if (check == calCheckSum(data,packet.getSeqnum(),ack)) {
			...
			if (WindowSize > 1) {
				...
				for(int i=0;i<WindowSize;i++){
					if(sW[i]!=null&&sW[i].getSeqnum()==ack){
						toLayer5(sW[i].getPayload());
					}
				}
				if (ack!=-1&&(ack <sW[0].getSeqnum()-1||ack==WindowSize-1)) {
				...
				if(sW[0]!=null&&ack!=sW[0].getSeqnum()-1&&rW[0]!=null){
					if(getSpace(sW)==0)
						stopTimer(0);
					slideWindow(sW,ack,true);
					slideWindow(rW,ack,false);
    ...
    protected void aTimerInterrupt()
    {
		...
		if (WindowSize == 1) {
			...
			for(int i=0;i<WindowSize;i++){
				...
				if(sW[i]!=null&&sW[i].getSeqnum()==lastACK+1){
					resentP=sW[i];
					break;
				}
			}
			toLayer3(0, resentP);
			System.out.println("Retransmitting packet " + (resentP.getSeqnum()));
		}
    ...
    protected void bInput(Packet packet)
    {
    	...
    	if(check==calCheckSum(data,seq,packet.getAcknum())){				//Means the packet is not corrupted
			if (WindowSize == 1) {
				...
				space = getSpace(rW);
				for(int i=0;i<WindowSize;i++){
					if(sW[i]!=null&&sW[i].getSeqnum()==seq){
						rW[i]=packet;
						break;
					}
				} 					// add packet to receiver window
			}
			if(WindowSize == 1){
				lastRCV = seq;
    		...
    		if(WindowSize>1){
    			if(seq == sW[0].getSeqnum()||seq==lastRCV+1){				//receive packet in correct order
    ...
    private void transmit(){
    	...
    	for(Packet p:pList){
    		...
    		if(WindowSize==1){
    			...
    			space = getSpace(sW);
    			sW[WindowSize-space]=p;
    			
    			System.out.println("Space: "+getSpace(sW));
    			if(getSpace(sW)==0){
    				T=false;
    				startTimer(0, RxmtInterval);
    				System.out.println("Timer is on Packet: "+p.getSeqnum());
    				rm.add(p);
    				break;
    			}
    ...
    private void slideWindow(Packet[] window, int n, boolean m){
    	if(n<window[0].getSeqnum())
    		n=n+8;
    	int x = n-window[0].getSeqnum();
    	for(int i=0;i<=x;i++){
    		if((window[i].getSeqnum()==WindowSize-1)&&m){
    			System.out.println("--------ACK OF WINDOW " + count + " IS FINISHED------------------------");
				count++;
    		}
    		window[i]=null;
    	}
    	for(int i=x+1;i<WindowSize;i++){
    		window[i-x-1]=window[i];
    		window[i]=null;
    	}
    	T = true;
    }
    private int getSpace(Packet[] window){
    	for(int i=0;i<WindowSize;i++){
    		if(window[i]==null){
    			return WindowSize-i;
    		}
